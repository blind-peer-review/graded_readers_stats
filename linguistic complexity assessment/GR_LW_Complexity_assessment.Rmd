---
title: 'Deconstructing the Library of Babel: Analysis of linguistic complexity in
  Spanish graded readers and literary works'
author: "Author 1"
date: "2023-07-31"
output:
...
readers_corpus <- dplyr::select(readers_corpus,
                                c(1, 2, (11:13), 16, 18, 20, 23, (25:27), 255,
                                  256, 258, 261, 263, 265, 267, 273, 372, 368,
                                  374, 370, 346, 348, 350, 416, 418, 420, 422,
                                  426, 428, 430, 651, 652, (657:659)))

# STEP 2. Add up auxiliary and regular verb features in a single column
...
# STEP 1. Select relevant columns
literature_corpus <- dplyr::select(literature_corpus,
                                   c(1, 2, (11:13), 16, 18, 20, 23, (25:27), 253,
                                     257, 256, 259, 260, 251, 262, 265, 384, 380,
                                     386, 382, 358, 360, 362, 502, 500, 504, 506,
                                     476, 478, 480, 637, 638, (643:645)))
...
# STEP 1. Select relevant columns
reference_corpus <- dplyr::select(reference_corpus,
                                  c((8:10), 13, 15, 17, 20, (22:24), 259, 260,
                                    265, 262, 264, 258, 269, 248, 591, 595, 593,
                                    613, 619, 617, 615, 449, 451, 453, 455, 425,
                                    427, 431, 650, 651, (656:658)))
...
nodes$name[1][1] <- "Graded readers (40.79%)"
nodes$name[2][1] <- "Literary works (39.82%)"
nodes$name[9][1] <- "Reference corpus (11.45%)"
...
GR_avgW <- GR_avgW + ylim(2.5, 12)
GR_ctxt_avgW <- GR_ctxt_avgW + ylim(2.5, 12)
GR_avgD <- GR_avgD + ylim(2.5, 7.5)
GR_ctxt_avgD <- GR_ctxt_avgD + ylim(2.5, 7.5)

LW_avgW <- LW_avgW + ylim(2, 15)
LW_ctxt_avgW <- LW_ctxt_avgW + ylim(2, 15)
LW_avgD <- LW_avgD + ylim(2, 9)
LW_ctxt_avgD <- LW_ctxt_avgD + ylim(2, 9)

RC_avgW <- RC_avgW + ylim(0, 25)
RC_ctxt_avgW <- RC_ctxt_avgW + ylim(0, 25)
RC_avgD <- RC_avgD + ylim(0, 15)
RC_ctxt_avgD <- RC_ctxt_avgD + ylim(0, 15)
...
```{r correlations, fig.width=12, fig.height=6, fig.align='default'}
par(mfrow=c(1,2))
...
readers_corr <- corrplot(readers_corr, type = "lower", order = "hclust",
                         title = "Graded readers", mar=c(0,0,1,0), tl.cex = 0.5,
...
literature_corr <- corrplot(literature_corr, type = "lower", order = "hclust",
                            title = "Literary works", mar=c(0,0,1,0), tl.cex = 0.5,
                            tl.col = "black", tl.srt = 45)
```
...
readers_model <- randomForest(Level ~ . - Title, data = readers_training, mtry=6, ntree=60,
...
literature_model <- randomForest(Level ~ . - Title, data = literature_training, mtry=6, ntree=75, importance=TRUE, proximity=TRUE)
literature_model
...
# If there is no difference in the mean average depth of the context trees of beginner GRs and LWs, we would observe a test statistic of 1.062 or more by chance roughly 0.02% of the time.  
mean(Perm.test.stat_GR_LW_ctxt_depth_A1A2 >= GR_LW_mean_ctxt_depth_stat_A1A2)
# If there is no difference in the mean average depth of the context trees of intermediate GRs and LWs, we would observe a test statistic of 0.02 or more by chance roughly 98.1% of the time.
mean(Perm.test.stat_GR_LW_ctxt_depth_B1 >= GR_LW_mean_ctxt_depth_stat_B1)
# If there is no difference in the mean average depth of the context trees of advanced GRs and LWs, we would observe a test statistic of 0.254 or more by chance roughly 72.87% of the time.
mean(Perm.test.stat_GR_LW_ctxt_depth_B2 >= GR_LW_mean_ctxt_depth_stat_B2)

# === GRs vs. RC ===

# If there is no difference in the mean average depth of the context trees of beginner GRs and RC, we would observe a test statistic of 2.96 or more by chance roughly 0% of the time.
mean(Perm.test.stat_GR_RC_ctxt_depth_A1A2 >= GR_RC_mean_ctxt_depth_stat_A1A2)
# If there is no difference in the mean average depth of the context trees of intermediate GRs and RC, we would observe a test statistic of 2.156 or more by chance roughly 73.2% of the time.
mean(Perm.test.stat_GR_RC_ctxt_depth_B1 >= GR_RC_mean_ctxt_depth_stat_B1)
# If there is no difference in the mean average depth of the context trees of advanced GRs and RC, we would observe a test statistic of 1.51 or more by chance roughly 99% of the time.
mean(Perm.test.stat_GR_RC_ctxt_depth_B2 >= GR_RC_mean_ctxt_depth_stat_B2)

# === LW vs. RC ===

# If there is no difference in the mean average depth of the context trees of beginner LW and RC, we would observe a test statistic of 1.9 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_ctxt_depth_A1A2 >= LW_RC_mean_ctxt_depth_stat_A1A2)
# If there is no difference in the mean average depth of the context trees of intermediate LW and RC, we would observe a test statistic of 2.18 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_ctxt_depth_B1 >= LW_RC_mean_ctxt_depth_stat_B1)
# If there is no difference in the mean average depth of the context trees of advanced LW and RC, we would observe a test statistic of 1.256 or more by chance roughly 0% of the time.
...
# If there is no difference in the mean average subjunctive frequencies of beginner GRs and LWs, we would observe a test statistic of 0.0037 or more by chance roughly 1.16% of the time.
mean(Perm.test.stat_GR_LW_sub_A1A2 >= GR_LW_mean_sub_stat_A1A2)
# If there is no difference in the mean average subjunctive frequencies of intermediate GRs and LWs, we would observe a test statistic of 0.002 or more by chance roughly 50.47% of the time.
mean(Perm.test.stat_GR_LW_sub_B1 >= GR_LW_mean_sub_stat_B1)
# If there is no difference in the mean average subjunctive frequencies of advanced GRs and LWs, we would observe a test statistic of 0.001 or more by chance roughly 79.86% of the time.
mean(Perm.test.stat_GR_LW_sub_B2 >= GR_LW_mean_sub_stat_B2)

# === GRs vs. RC ===

# If there is no difference in the mean average subjunctive frequencies of beginner GRs and RC, we would observe a test statistic of 0.0038 or more by chance roughly 0% of the time.
mean(Perm.test.stat_GR_RC_sub_A1A2 >= GR_RC_mean_sub_stat_A1A2)
# If there is no difference in the mean average subjunctive frequencies of intermediate GRs and RC, we would observe a test statistic of 0.0011 or more by chance roughly 62.55% of the time.
mean(Perm.test.stat_GR_RC_sub_B1 >= GR_RC_mean_sub_stat_B1)
# If there is no difference in the mean average subjunctive frequencies of advanced GRs and RC, we would observe a test statistic of 0.0013 or more by chance roughly 52.51% of the time.
mean(Perm.test.stat_GR_RC_sub_B2 >= GR_RC_mean_sub_stat_B2)

# === LW vs. RC ===

# If there is no difference in the mean average subjunctive frequencies of beginner LW and RC, we would observe a test statistic of 3.0127e-05 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_sub_A1A2 >= LW_RC_mean_sub_stat_A1A2)
# If there is no difference in the mean average subjunctive frequencies of intermediate LW and RC, we would observe a test statistic of 0.00083 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_sub_B1 >= LW_RC_mean_sub_stat_B1)
# If there is no difference in the mean average subjunctive frequencies of advanced LW and RC, we would observe a test statistic of 0.0024 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_sub_B2 >= LW_RC_mean_sub_stat_B2)

# --------------------

# 3) RELATIVE FREQUENCY OF B1 WORDS

# === Step 1. Calculate the difference in sample means: ===

GR_mean_B1_A1A2 <- mean(readers_corpus$Freq_B1[readers_corpus$Level=="Beginner"])
GR_mean_B1_B1 <- mean(readers_corpus$Freq_B1[readers_corpus$Level=="Intermediate"])
GR_mean_B1_B2 <- mean(readers_corpus$Freq_B1[readers_corpus$Level=="Advanced"])

LW_mean_B1_A1A2 <- mean(literature_corpus$Freq_B1[literature_corpus$Level=="Children's literature"])
LW_mean_B1_B1 <- mean(literature_corpus$Freq_B1[literature_corpus$Level=="Young adult fiction"])
LW_mean_B1_B2 <- mean(literature_corpus$Freq_B1[literature_corpus$Level=="Adult literature"])

RC_mean_B1 <- mean(reference_corpus$Freq_B1)

# === Step 2. Calculate the absolute difference in means ===

GR_LW_mean_B1_stat_A1A2 <- abs(GR_mean_B1_A1A2 - LW_mean_B1_A1A2)
GR_LW_mean_B1_stat_B1 <- abs(GR_mean_B1_B1 - LW_mean_B1_B1)
GR_LW_mean_B1_stat_B2 <- abs(GR_mean_B1_B2 - LW_mean_B1_B2)

GR_RC_mean_B1_stat_A1A2 <- abs(GR_mean_B1_A1A2 - RC_mean_B1)
GR_RC_mean_B1_stat_B1 <- abs(GR_mean_B1_B1 - RC_mean_B1)
GR_RC_mean_B1_stat_B2 <- abs(GR_mean_B1_B2 - RC_mean_B1)

LW_RC_mean_B1_stat_A1A2 <- abs(LW_mean_B1_A1A2 - RC_mean_B1)
LW_RC_mean_B1_stat_B1 <- abs(LW_mean_B1_B1 - RC_mean_B1)
LW_RC_mean_B1_stat_B2 <- abs(LW_mean_B1_B2 - RC_mean_B1)

# === Step 3. Perform the permutation test ===

# Variable we will resample from
GR_B1_variable <- readers_corpus$Freq_B1
LW_B1_variable <- literature_corpus$Freq_B1
RC_B1_variable <- reference_corpus$Freq_B1

# Initialize a matrix to store the permutation data. Each column is a permutation sample of data.
GR_B1_PermSamples <- matrix(0, nrow=GR_n, ncol=P)
LW_B1_PermSamples <- matrix(0, nrow=LW_n, ncol=P)
RC_B1_PermSamples <- matrix(0, nrow=RC_n, ncol=P)

for(i in 1:P){
  GR_B1_PermSamples[, i] <- sample(GR_B1_variable, size=GR_n, replace=FALSE)
}

for(i in 1:P){
  LW_B1_PermSamples[, i] <- sample(LW_B1_variable, size=LW_n, replace=FALSE)
}

for(i in 1:P){
  RC_B1_PermSamples[, i] <- sample(RC_B1_variable, size=RC_n, replace=FALSE)
}

# Initialize vectors to store the test stats
Perm.test.stat_GR_LW_B1_A1A2 <- rep(0, P)
Perm.test.stat_GR_LW_B1_B1 <- rep(0, P)
Perm.test.stat_GR_LW_B1_B2 <- rep(0, P)

Perm.test.stat_GR_RC_B1_A1A2 <- rep(0, P)
Perm.test.stat_GR_RC_B1_B1 <- rep(0, P)
Perm.test.stat_GR_RC_B1_B2 <- rep(0, P)

Perm.test.stat_LW_RC_B1_A1A2 <- rep(0, P)
Perm.test.stat_LW_RC_B1_B1 <- rep(0, P)
Perm.test.stat_LW_RC_B1_B2 <- rep(0, P)

# Loop through, and calculate test stats
for(i in 1:P){
  Perm.test.stat_GR_LW_B1_A1A2[i] <- abs(mean(GR_B1_PermSamples[
    readers_corpus$Level=="Beginner", i]) - mean(LW_B1_PermSamples[
      literature_corpus$Level=="Children's literature", i]))
}
for(i in 1:P){
  Perm.test.stat_GR_LW_B1_B1[i] <- abs(mean(GR_B1_PermSamples[
    readers_corpus$Level=="Intermediate", i]) - mean(LW_B1_PermSamples[
      literature_corpus$Level=="Young adult fiction", i]))
}
for(i in 1:P){
  Perm.test.stat_GR_LW_B1_B2[i] <- abs(mean(GR_B1_PermSamples[
    readers_corpus$Level=="Advanced", i]) - mean(LW_B1_PermSamples[
      literature_corpus$Level=="Adult literature", i]))
}

for(i in 1:P){
  Perm.test.stat_GR_RC_B1_A1A2[i] <- abs(mean(GR_B1_PermSamples[
    readers_corpus$Level=="Beginner", i]) - mean(RC_B1_PermSamples[
      reference_corpus$group==1, i]))
}
for(i in 1:P){
  Perm.test.stat_GR_RC_B1_B1[i] <- abs(mean(GR_B1_PermSamples[
    readers_corpus$Level=="Intermediate", i]) - mean(RC_B1_PermSamples[
      reference_corpus$group==1, i]))
}
for(i in 1:P){
  Perm.test.stat_GR_RC_B1_B2[i] <- abs(mean(GR_B1_PermSamples[
    readers_corpus$Level=="Advanced", i]) - mean(RC_B1_PermSamples[
      reference_corpus$group==1, i]))
}
...
# If there is no difference in the mean average B1 word frequencies of beginner GRs and LWs, we would observe a test statistic of 0.0028 or more by chance roughly 38.72% of the time.
mean(Perm.test.stat_GR_LW_B1_A1A2 >= GR_LW_mean_B1_stat_A1A2)
# If there is no difference in the mean average B1 word frequencies of intermediate GRs and LWs, we would observe a test statistic of 0.00057 or more by chance roughly 86.64% of the time.
mean(Perm.test.stat_GR_LW_B1_B1 >= GR_LW_mean_B1_stat_B1)
# If there is no difference in the mean average B1 word frequencies of advanced GRs and LWs, we would observe a test statistic of 0.0019 or more by chance roughly 62% of the time.
mean(Perm.test.stat_GR_LW_B1_B2 >= GR_LW_mean_B1_stat_B2)

# === GRs vs. RC ===

# If there is no difference in the mean average B1 word frequencies of beginner GRs and RC, we would observe a test statistic of 0.0088 or more by chance roughly 8.6% of the time.
mean(Perm.test.stat_GR_RC_B1_A1A2 >= GR_RC_mean_B1_stat_A1A2)
# If there is no difference in the mean average B1 word frequencies of intermediate GRs and RC, we would observe a test statistic of 0.008 or more by chance roughly 16.7% of the time.
mean(Perm.test.stat_GR_RC_B1_B1 >= GR_RC_mean_B1_stat_B1)
# If there is no difference in the mean average B1 word frequencies of advanced GRs and RC, we would observe a test statistic of 0.002 or more by chance roughly 99.55% of the time.
mean(Perm.test.stat_GR_RC_B1_B2 >= GR_RC_mean_B1_stat_B2)

# === LW vs. RC ===

# If there is no difference in the mean average B1 word frequencies of beginner LW and RC, we would observe a test statistic of 0.011 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_B1_A1A2 >= LW_RC_mean_B1_stat_A1A2)
# If there is no difference in the mean average B1 word frequencies of intermediate LW and RC, we would observe a test statistic of 0.0086 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_B1_B1 >= LW_RC_mean_B1_stat_B1)
# If there is no difference in the mean average B1 word frequencies of advanced LW and RC, we would observe a test statistic of 0.00014 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_B1_B2 >= LW_RC_mean_B1_stat_B2)

# --------------------

# 4) RELATIVE FREQUENCY OF THE GRADED VOCABULARY'S CONTEXT

# === Step 1. Calculate the difference in sample means: ===

GR_mean_ctxt_freq_A1A2 <- mean(readers_corpus$Ctxt_Freq[readers_corpus$Level=="Beginner"])
GR_mean_ctxt_freq_B1 <- mean(readers_corpus$Ctxt_Freq[readers_corpus$Level=="Intermediate"])
GR_mean_ctxt_freq_B2 <- mean(readers_corpus$Ctxt_Freq[readers_corpus$Level=="Advanced"])

LW_mean_ctxt_freq_A1A2 <- mean(literature_corpus$Ctxt_Freq[literature_corpus$Level=="Children's literature"])
LW_mean_ctxt_freq_B1 <- mean(literature_corpus$Ctxt_Freq[literature_corpus$Level=="Young adult fiction"])
LW_mean_ctxt_freq_B2 <- mean(literature_corpus$Ctxt_Freq[literature_corpus$Level=="Adult literature"])

RC_mean_ctxt_freq <- mean(reference_corpus$Ctxt_Freq)

# === Step 2. Calculate the absolute difference in means ===

GR_LW_mean_ctxt_freq_stat_A1A2 <- abs(GR_mean_ctxt_freq_A1A2 - LW_mean_ctxt_freq_A1A2)
GR_LW_mean_ctxt_freq_stat_B1 <- abs(GR_mean_ctxt_freq_B1 - LW_mean_ctxt_freq_B1)
GR_LW_mean_ctxt_freq_stat_B2 <- abs(GR_mean_ctxt_freq_B2 - LW_mean_ctxt_freq_B2)

GR_RC_mean_ctxt_freq_stat_A1A2 <- abs(GR_mean_ctxt_freq_A1A2 - RC_mean_ctxt_freq)
GR_RC_mean_ctxt_freq_stat_B1 <- abs(GR_mean_ctxt_freq_B1 - RC_mean_ctxt_freq)
GR_RC_mean_ctxt_freq_stat_B2 <- abs(GR_mean_ctxt_freq_B2 - RC_mean_ctxt_freq)

LW_RC_mean_ctxt_freq_stat_A1A2 <- abs(LW_mean_ctxt_freq_A1A2 - RC_mean_ctxt_freq)
LW_RC_mean_ctxt_freq_stat_B1 <- abs(LW_mean_ctxt_freq_B1 - RC_mean_ctxt_freq)
LW_RC_mean_ctxt_freq_stat_B2 <- abs(LW_mean_ctxt_freq_B2 - RC_mean_ctxt_freq)

# === Step 3. Perform the permutation test ===

# Variable we will resample from
GR_ctxt_freq_variable <- readers_corpus$Ctxt_Freq
LW_ctxt_freq_variable <- literature_corpus$Ctxt_Freq
RC_ctxt_freq_variable <- reference_corpus$Ctxt_Freq

# Initialize a matrix to store the permutation data. Each column is a permutation sample of data.
GR_ctxt_freq_PermSamples <- matrix(0, nrow=GR_n, ncol=P)
LW_ctxt_freq_PermSamples <- matrix(0, nrow=LW_n, ncol=P)
RC_ctxt_freq_PermSamples <- matrix(0, nrow=RC_n, ncol=P)

for(i in 1:P){
  GR_ctxt_freq_PermSamples[, i] <- sample(GR_ctxt_freq_variable, size=GR_n, replace=FALSE)
}

for(i in 1:P){
  LW_ctxt_freq_PermSamples[, i] <- sample(LW_ctxt_freq_variable, size=LW_n, replace=FALSE)
}

for(i in 1:P){
  RC_ctxt_freq_PermSamples[, i] <- sample(RC_ctxt_freq_variable, size=RC_n, replace=FALSE)
}

# Initialize vectors to store the test stats
Perm.test.stat_GR_LW_ctxt_freq_A1A2 <- rep(0, P)
Perm.test.stat_GR_LW_ctxt_freq_B1 <- rep(0, P)
Perm.test.stat_GR_LW_ctxt_freq_B2 <- rep(0, P)

Perm.test.stat_GR_RC_ctxt_freq_A1A2 <- rep(0, P)
Perm.test.stat_GR_RC_ctxt_freq_B1 <- rep(0, P)
Perm.test.stat_GR_RC_ctxt_freq_B2 <- rep(0, P)

Perm.test.stat_LW_RC_ctxt_freq_A1A2 <- rep(0, P)
Perm.test.stat_LW_RC_ctxt_freq_B1 <- rep(0, P)
Perm.test.stat_LW_RC_ctxt_freq_B2 <- rep(0, P)

# Loop through, and calculate test stats
for(i in 1:P){
  Perm.test.stat_GR_LW_ctxt_freq_A1A2[i] <- abs(mean(GR_ctxt_freq_PermSamples[
    readers_corpus$Level=="Beginner", i]) - mean(LW_ctxt_freq_PermSamples[
      literature_corpus$Level=="Children's literature", i]))
}
for(i in 1:P){
  Perm.test.stat_GR_LW_ctxt_freq_B1[i] <- abs(mean(GR_ctxt_freq_PermSamples[
    readers_corpus$Level=="Intermediate", i]) - mean(LW_ctxt_freq_PermSamples[
      literature_corpus$Level=="Young adult fiction", i]))
}
for(i in 1:P){
  Perm.test.stat_GR_LW_ctxt_freq_B2[i] <- abs(mean(GR_ctxt_freq_PermSamples[
    readers_corpus$Level=="Advanced", i]) - mean(LW_ctxt_freq_PermSamples[
      literature_corpus$Level=="Adult literature", i]))
}

for(i in 1:P){
  Perm.test.stat_GR_RC_ctxt_freq_A1A2[i] <- abs(mean(GR_ctxt_freq_PermSamples[
    readers_corpus$Level=="Beginner", i]) - mean(RC_ctxt_freq_PermSamples[
      reference_corpus$group==1, i]))
}
for(i in 1:P){
  Perm.test.stat_GR_RC_ctxt_freq_B1[i] <- abs(mean(GR_ctxt_freq_PermSamples[
    readers_corpus$Level=="Intermediate", i]) - mean(RC_ctxt_freq_PermSamples[
      reference_corpus$group==1, i]))
}
for(i in 1:P){
  Perm.test.stat_GR_RC_ctxt_freq_B2[i] <- abs(mean(GR_ctxt_freq_PermSamples[
    readers_corpus$Level=="Advanced", i]) - mean(RC_ctxt_freq_PermSamples[
      reference_corpus$group==1, i]))
}
...
# If there is no difference in the mean average graded vocabulary's context frequencies of beginner GRs and LWs, we would observe a test statistic of 0.00058 or more by chance roughly 1.24% of the time.
mean(Perm.test.stat_GR_LW_ctxt_freq_A1A2 >= GR_LW_mean_ctxt_freq_stat_A1A2)
# If there is no difference in the mean average graded vocabulary's context frequencies of intermediate GRs and LWs, we would observe a test statistic of 0.00048 or more by chance roughly 22.11% of the time.
mean(Perm.test.stat_GR_LW_ctxt_freq_B1 >= GR_LW_mean_ctxt_freq_stat_B1)
# If there is no difference in the mean average graded vocabulary's context frequencies of advanced GRs and LWs, we would observe a test statistic of 0.00081 or more by chance roughly 4.5% of the time.
mean(Perm.test.stat_GR_LW_ctxt_freq_B2 >= GR_LW_mean_ctxt_freq_stat_B2)

# === GRs vs. RC ===

# If there is no difference in the mean average graded vocabulary's context frequencies of beginner GRs and RC, we would observe a test statistic of 0.0048 or more by chance roughly 84.58% of the time.
mean(Perm.test.stat_GR_RC_ctxt_freq_A1A2 >= GR_RC_mean_ctxt_freq_stat_A1A2)
# If there is no difference in the mean average graded vocabulary's context frequencies of intermediate GRs and RC, we would observe a test statistic of 0.0051 or more by chance roughly 21.78% of the time.
mean(Perm.test.stat_GR_RC_ctxt_freq_B1 >= GR_RC_mean_ctxt_freq_stat_B1)
# If there is no difference in the mean average graded vocabulary's context advanced of beginner GRs and RC, we would observe a test statistic of 0.005 or more by chance roughly 42.44% of the time.
mean(Perm.test.stat_GR_RC_ctxt_freq_B2 >= GR_RC_mean_ctxt_freq_stat_B2)

# === LWs vs. RC ===

# If there is no difference in the mean average graded vocabulary's context frequencies of beginner LWs and RC, we would observe a test statistic of 0.0043 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_ctxt_freq_A1A2 >= LW_RC_mean_ctxt_freq_stat_A1A2)
# If there is no difference in the mean average graded vocabulary's context frequencies of intermediate LWs and RC, we would observe a test statistic of 0.0056 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_ctxt_freq_B1 >= LW_RC_mean_ctxt_freq_stat_B1)
# If there is no difference in the mean average graded vocabulary's context frequencies of advanced LWs and RC, we would observe a test statistic of 0.0058 or more by chance roughly 0% of the time.
mean(Perm.test.stat_LW_RC_ctxt_freq_B2 >= LW_RC_mean_ctxt_freq_stat_B2)
